...
Il **filtro Variazione colore** consente di modificare rapidamente più colori del canale del colore di base contemporaneamente. Per alcuni materiali questo può essere utile per effettuare piccole regolazioni, ma il **filtro Variazione colore** è il più adatto per rivedere completamente i colori del materiale con un singolo filtro.

Per utilizzare il **filtro Variazione colore**:

1. Aggiungi il **filtro Variazione colore** allo stack di livelli
...
1. Selezionate una **modalità di selezione colore**. Con la modalità manuale selezionata, nella **vista 2D** vengono visualizzate le maniglie che consentono di selezionare il colore di base originale che verrà sostituito dal nuovo colore. Abilita **Mostra Helper testo** per tenere traccia della maniglia collegata al colore.
